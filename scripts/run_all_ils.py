...
def run_all(relaxation, use_setup_times):
    ...
    for n in [8, 16, 32, 64]: #, 128]:
        for q in [2**i for i in range(0, 10) if 2**i <= n]:
            ...
            # Choose the `tunned` perturbation strength
            if pscenario == "routing-scheduling" and relaxation == 0:
                perturbation = 0.61
            elif pscenario == "routing-scheduling" and relaxation == 0.5:
                perturbation = 0.19
            elif pscenario == "routing-scheduling" and relaxation == 1:
                perturbation = 0.86
