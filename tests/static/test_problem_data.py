...
from optlis.shared import _write_solution
from optlis.static.problem_data import load_instance, _write_instance


def test_Graph(example_instance):
    """Tests the Instance class."""
    assert np.array_equal(example_instance.depots, np.array([0]))
    assert np.array_equal(example_instance.tasks, np.array([1, 2, 3, 4, 5, 6, 7, 8]))


@pytest.mark.parametrize(["time_horizon", "T"], [(None, 76), (70, 69), (15, 14)])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0.3, [(8, 1), (8, 2), (8, 3), (7, 1), (7, 2), (6, 1), (5, 1)]),
        (0.5, [(8, 1)]),
        (1, []),
    ],
)
