...
def build(c):
    ...
    try:
        c.run("gcc -c -fPIC optlis/solvers/localsearch.c -o "
              f"{build_dir / 'localsearch.o'}")
        c.run(f"gcc -shared -Wl,-soname,localsearch.so -o "
...
def bulk_solve(c, method, inst_dir, relaxation=0.0, perturbation=0.5, stop=0, repeat=35,
               parallel=4, tt_off=False, log_dir=None, sol_dir=None):
    """Solves all instances located in the 'inst-dir' directory."""
    if method.lower() == "ils":
        _bulk_solve_ils(inst_dir, relaxation, perturbation,
                        stop, repeat, parallel, tt_off)
