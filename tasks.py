...
def _bulk_solve_dynamic_ils(inst_dir, stop, repeat, parallel, log_dir):
    ...
    for i, path in enumerate(inst_paths, start=1):
        ...
        if "-ab-" in path.stem or "-ba-" in path.stem:
            benchmark = "no-degradation"
        elif "-asb-" in path.stem or "-bsa-" in path.stem:
            benchmark = "parent-degradation"
        elif "-abs-" in path.stem or "-bas-" in path.stem:
            benchmark = "metabolite-degradation"
        else:
            benchmark = "parent-metabolite-degradation"
        ...
        rho1, rho2 = _get_irace_dynamic_config(size, scenario, benchmark)
...
def _get_irace_dynamic_config(size, scenario, benchmark):
    """These values were separately generated by the irace package and are hardcoded
    here for the purpuse of repeatability of results.
    """
