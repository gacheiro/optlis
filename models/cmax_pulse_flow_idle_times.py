...
def make_prob(G, relaxation_threshold=0.0, no_graph=False):
    ...
    sd = plp.LpVariable.dicts("sd", indexs=D, lowBound=0, cat=plp.LpContinuous)
    cd = plp.LpVariable.dicts("cd", indexs=D, lowBound=0, cat=plp.LpContinuous)
    y = plp.LpVariable.dicts("y", indexs=(V, V, T), lowBound=0, cat=plp.LpInteger)

    # The objective function
    prob = plp.LpProblem("Cmax_Pulse_Flow_Idle", plp.LpMinimize)
    ...
    for j in D:
        prob += (
            cd[j] == plp.lpSum((t + c[i][j]) * y[i][j][t] for t in T
                                                          for i in V if i != j)
                    + p[j]), f"R7_Completion_date_of_{j}"
