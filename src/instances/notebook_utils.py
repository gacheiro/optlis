...
def plot_gantt_diagram(instance_path, sol_path):
    ...
    G, sol = (load_instance(instance_path),
              import_solution(sol_path))

    plt.rcdefaults()
    fig, ax = plt.subplots()

    jobs = sorted(G.destinations, key=lambda x: G.nodes[x]["r"], reverse=True)
    start_dates = [sol.get(f"sd_{j}") for j in jobs]
    durations = [G.nodes[j]["p"] for j in jobs]
...
if __name__ == "__main__":
    ...
    plot_gantt_diagram(
        instance_path="data/instances/tn_n16_p7_q2.dat",
        sol_path="data/solutions/m1/tn_n16_p7_q2.sol",
    )

    plot_gantt_diagram(
        instance_path="data/instances/tn_n16_p7_q2.dat",
        sol_path="data/solutions/m2/tn_n16_p7_q2.sol",
    )
