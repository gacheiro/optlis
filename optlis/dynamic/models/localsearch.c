...
void degradation_scheme(const struct instance *inst, struct solution *sol, size_t t) {
...
void metabolizing_scheme(const struct instance *inst, struct solution *sol, size_t t) {
  ...
  for (size_t i = 0; i < inst->nnodes; ++i) {
    for (size_t p = 0; p < inst->nproducts; ++p) {
      for (size_t q = 0; q < inst->nproducts; ++q) {
        double lastc = sol->nodes_concentration[(t - 1) + ntime_units * (p + nproducts * i)];
        double amount = (lastc - lastc * inst->degradation_rates[p]) * inst->metabolizing_rates[p * nproducts + q];

        sol->nodes_concentration[t + ntime_units * (p + nproducts * i)] -= amount;
        sol->nodes_concentration[t + ntime_units * (q + nproducts * i)] += amount;
...
void neutralizing_scheme(const struct instance *inst, struct solution *sol, size_t i, size_t p, size_t t) {
    
  int nproducts = inst->nproducts;
  int ntime_units = inst->ntime_units;

  double lastc = sol->nodes_concentration[(t - 1) + ntime_units * (p + nproducts * i)];
  double amount = NEUTRALIZING_SPEED * lastc - lastc * inst->degradation_rates[p];
...
void cleaning_scheme(const struct instance *inst, struct solution *sol, size_t i, size_t t) {
...
//       int start_time = inst->cleaning_start_times[curr_task_id*ntime_units + time_unit];
...
//         cleaning_scheme(inst, sol, curr_task_id, time_unit);
//         continue;
//       }
...
//         cleaning_scheme(inst, sol, task_id, time_unit);
